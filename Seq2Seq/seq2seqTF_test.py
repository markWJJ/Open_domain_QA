...
class Seq2Seq(Seq2SeqBasic):
    ...
    def __init_ops__(self):
        ...
        with tf.variable_scope('embedding'):
            self.embeding_content=tf.get_variable(shape=(self.content_vocab_len,self.init_dim),dtype=tf.float32,
                                                  initializer=tf.random_normal_initializer(),name="embeding_content")
            self.embeding_title = tf.get_variable(shape=(self.title_vocab_len, self.init_dim), dtype=tf.float32,
                                                    initializer=tf.random_normal_initializer(),name="embeding_title")
            # # 构建encoder层词向量矩阵变量
    ...
    def __encoder__(self,encoder_inputs,encoder_len,reuse=None,**kwargs):
        '''
        编码层
        :return: 
        '''
    ...
    def __decoder__(self,encoder_outs,encoder_state_c,encoder_state_h,reuse=None,**kwargs):
        '''
    ...
    def __loss__(self,decoder_outs,label,reuse=False,**kwargs):
        '''
                ...
                if not FLAGS.sample_loss:
                    ...
                    def sampled_loss_func(labels, inputs):
                        ...
                        return tf.nn.sampled_softmax_loss(
                            weights=self.w, biases=b, labels=labels, inputs=inputs,
    ...
    def __train_ops__(self):
        ...
        encoder_outs, encoder_state_c, encoder_state_h = self.__encoder__(self.title_emb,self.title_len,reuse=None,**kwargs_encoder)
        ...
        # if index==0:
        decoder_out, decoder_state = self.__decoder__(encoder_outs, encoder_state_c, encoder_state_h,reuse=False,**kwargs_decoder)
        # else:
        #     decoder_out, decoder_state = self.__decoder__(encoder_outs, encoder_state_c, encoder_state_h,reuse=True,**kwargs_decoder)
        ...
        loss_kwargs={"num_class":self.num_class,
                     ...
                     "decoder_len":self.content_len}
        # if index==0:
        self.loss,_ = self.__loss__(decoder_out, self.content_decoder,reuse=False,**loss_kwargs)
        # else:
        #     self.loss,_ = self.__loss__(decoder_out, self.content_decoder,reuse=True,**loss_kwargs)


        tf.summary.scalar("loss_my", self.loss)
        # if index==0:
        with tf.variable_scope("opt",reuse=False):
    ...
    def __decoder_ops__(self):
        ...
        with tf.variable_scope("embedding",reuse=True):
            embeding_content=tf.get_variable(name="embeding_content",shape=(self.content_vocab_len,self.init_dim))
    ...
    def Train(self,*args):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(FLAGS.epoch):
                ...
                if train_flag == 'train':
                    su_merge, loss, _,outs = sess.run([self.merge_summary, self.loss, self.opt,self.encoder_outs], feed_dict={
...
def main(_):
    ...
    model = Seq2Seq(hidden_dim=FLAGS.hidden_dim, init_dim=FLAGS.embedding_dim, content_len=FLAGS.decoder_len,
                          title_len=FLAGS.encoder_len,
                          con_vocab_len=content_vocab_size, ti_vocab_len=title_vocab_size,
                          batch_size=FLAGS.batch_size)

    if FLAGS.mod=='train':
        _logger.info("开始第%s轮训练"%0)
        model.Train(dd)
