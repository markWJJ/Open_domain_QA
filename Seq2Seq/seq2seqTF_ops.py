...
def embedding_rnn_decoder(decoder_inputs,
    ...
    with variable_scope.variable_scope(scope or "embedding_rnn_decoder") as scope:
        ...
        # loop_function 是计算输入值的最大下标，为什么只有预测的时候需要？因为预测时需要前一个节点的输出作为输出，所以需要获取前一个节点的最大可能的下标
...
def embedding_rnn_seq2seq(encoder_inputs,
    ...
    Args:
      ...
      cell: core_rnn_cell.RNNCell defining the cell function and size.
      num_encoder_symbols: Integer; number of symbols on the encoder side.# 编码词典
      num_decoder_symbols: Integer; number of symbols on the decoder side. # 解码词典
      embedding_size: Integer, the length of the embedding vector for each symbol. # 词嵌入大小
      output_projection: None or a pair (W, B) of output projection weights and # 输出映射
        biases; W has shape [output_size x num_decoder_symbols] and B has 
        shape [num_decoder_symbols]; if provided and feed_previous=True, each
        fed previous output will first be multiplied by W and added B.
      feed_previous: Boolean or scalar Boolean Tensor; if True, only the first  # 是否预测 如果是 则第一个decoder的输入为标签 其他的节点的输入为前一个节点的输出
    ...
    Returns:
      A tuple of the form (outputs, state), where:           #输出的维度 由 output_projection的W决定
        outputs: A list of the same length as decoder_inputs of 2D Tensors. The  
          output is of shape [batch_size x cell.output_size] when
          output_projection is not None (and represents the dense representation
          of predicted tokens). It is of shape [batch_size x num_decoder_symbols]
          when output_projection is None.
        state: The state of each decoder cell in each time-step. This is a list  #每个cell 的单元状态值
          with length len(decoder_inputs) -- one item for each time-step.
          It is a 2D Tensor of shape [batch_size x cell.state_size].
    """
    with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
        ...
        # Decoder. 如果输出映射存在 则进行输出映射 解码和编码使用同一类神经网络
...
def embedding_encoder(encoder_inputs,
    ...
    # 构建attention states size=[batch_size,encoder_length,cell.output_size]
