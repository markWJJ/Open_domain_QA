...
from Data_r_net import Data_deal
...
class R_Net(object):
    ...
    R_net 智能问答模型
    '''
    def __init__(self,init_dim,hidden_dim,Q_len,P_len,batch_size):
        ...
        self.P=tf.placeholder(dtype=tf.float32,shape=(None,self.P_len,self.init_dim))
        ...
        self.Q_,_ = self.process(input_=self.Q,seq_len=self.Q_len,seq_vec=self.Q_seq_vec,scope="Q_encoder")
        self.P_,_ = self.process(input_=self.P,seq_len=self.P_len,seq_vec=self.P_seq_vec,scope="P_encoder")
...
if __name__ == '__main__':
    ...
    P_len=100
    hidden_dim=100
    dd= Data_deal.DataDealRNet(train_path="/SQUQA_train_1.txt", test_path="/test1.txt",
                               dev_path="/dev1.txt",
                               dim=init_dim, batch_size=batch_size, Q_len=Q_len, P_len=P_len, flag="train_new")
    ca=R_Net(init_dim=init_dim,hidden_dim=hidden_dim,Q_len=Q_len,P_len=P_len,batch_size=batch_size)
    ca.train(dd)
